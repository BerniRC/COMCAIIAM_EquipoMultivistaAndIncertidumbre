...
change.method.names <- function(dataset_path, dataset_name = "htad", view_names = NULL) {
  ...
  if (is.null(view_names)) {
    ...
    else if (dataset_name == "results_1") {
      view_names <- c(
        v1 = "Context",
        v2 = "NetworkFlow",
        v3 = "log"
  ...
  for (view_code in names(view_names)) {
    results$method[results$method == view_code] <- view_names[[view_code]]
  }
  ...
  write.csv(
    results,
    paste0(dataset_path, "results_1//results_mvc.csv"),
    row.names = FALSE,
    quote = FALSE
  )
}
...
add.intersection.model <- function(dataset_path){
  ...
  for(i in 1:nrow(df.v1)){
    ...
    avg.scores[is.na(avg.scores)] <- 0
    avg.scores[is.infinite(avg.scores)] <- 0
...
add.intersection.model.2v <- function(dataset_path){
  # Function that adds the predictions for the intersection model.
  # Versión optimizada para no usar rbind dentro del ciclo.

  results <- read.csv(paste0(dataset_path, "results_1//results.csv"))

  labels.order <- read.csv(
    paste0(dataset_path, "classes.csv"),
    header = FALSE
  )[, 1]
  ...
  if (n != nrow(df.v2)) {
    stop("v1 y v2 no tienen la misma cantidad de filas.")
  }

  nuevas_filas <- vector("list", n)

  for (i in 1:n) {

    if (i %% 1000 == 0) {
      cat("Procesando fila", i, "de", n, "\n")
    }
    ...
    ps1 <- strsplit(as.character(row.v1$predictionSet), "\\|")[[1]]

    if (length(ps1) == 0 || ps1[1] == "") {
      ps1 <- ""
    } else {
      scores1 <- as.numeric(strsplit(as.character(row.v1$scores), "\\|")[[1]])
      sum.scores[ps1] <- scores1
      counts[ps1] <- counts[ps1] + 1
    }

    ps2 <- strsplit(as.character(row.v2$predictionSet), "\\|")[[1]]

    if (length(ps2) == 0 || ps2[1] == "") {
      ps2 <- ""
    } else {
      scores2 <- as.numeric(strsplit(as.character(row.v2$scores), "\\|")[[1]])
      sum.scores[ps2] <- sum.scores[ps2] + scores2
      counts[ps2] <- counts[ps2] + 1
    }
    ...
    pvs1 <- as.numeric(strsplit(as.character(row.v1$pvalues), "\\|")[[1]])
    pvs2 <- as.numeric(strsplit(as.character(row.v2$pvalues), "\\|")[[1]])
  ...
  nuevas_filas <- do.call(rbind, nuevas_filas)

  all <- rbind(results, nuevas_filas)
